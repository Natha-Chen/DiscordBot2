...
class YoutubeHook(commands.Cog):
	...
	async def web_server(self):
		...
		async def receive(request):
			...
			if 'entry' in data['feed'] and data['feed']['entry']['yt:videoId'] not in self.memory:
				...
				self.client.dispatch('new_video', video_data)
	...
	async def subscribe(self):
		...
		async with ClientSession() as session:
			...
			payload = {
				'hub.callback': self.client.config['callback_url'],
				'hub.mode': 'subscribe',
				'hub.topic': f'https://www.youtube.com/xml/feeds/videos.xml?channel_id={channel_id}',
				'hub.lease_seconds': '828000', # Might want to define this, max 828000 seconds
				'hub.secret': '',
				'hub.verify': 'async',
				'hub.verify_token': ''
			}
